...
func reactToEvent(line string) {
	...
	if achivement := RegexPlayerAchievement.FindStringSubmatch(line); len(achivement) > 0 {
		dg.SendDeduplicatedMsg(discorder.NewDeduplicater(types.DockerTimestamp(achivement[1])), fmt.Sprintf("[%s] player %s has just earned the achievement %s", achivement[1], achivement[2], achivement[3]), shared_settings.Channel)
	}
}
